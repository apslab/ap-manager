class MaritalStatusesController < ApplicationController
  ...
  def create
	...
	flash[:notice] = "Marital Status creado con exito." if @marital_status.save
    respond_with(@marital_status)
  end
  ...
  def update
    @marital_status = MaritalStatus.find(params[:id])
	flash[:notice] = "Marital Status actualizado con exito." if @marital_status.update_attributes(params[:marital_status])
  end

  def destroy
    @marital_status = MaritalStatus.find(params[:id])
    @marital_status.destroy
	flash[:notice] = "Marital Status borrado con exito." if @marital_status.destroy
    respond_with(@marital_status)
  end
end
