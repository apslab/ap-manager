class CareTypesController < ApplicationController
  ...
  def create
	...
	flash[:notice] = "Care Type creado con exito." if @care_type.save
    respond_with(@care_type)
  end
  ...
  def update
    @care_type = CareType.find(params[:id])
	flash[:notice] = "Care Type actualizado con exito." if @care_type.update_attributes(params[:care_type])
  end

  def destroy
    @care_type = CareType.find(params[:id])
    @care_type.destroy
	flash[:notice] = "Care Type borrado con exito." if @care_type.destroy
    respond_with(@care_type)
  end
end
