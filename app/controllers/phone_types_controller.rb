class PhoneTypesController < ApplicationController
  ...
  def create
	...
	flash[:notice] = "Phone Type creado con exito." if @phone_type.save
    respond_with(@phone_type)
  end
  ...
  def update
    @phone_type = PhoneType.find(params[:id])
	flash[:notice] = "Phone Type actualizado con exito." if @phone_type.update_attributes(params[:phone_type])
  end

  def destroy
    @phone_type = PhoneType.find(params[:id])
    @phone_type.destroy
	flash[:notice] = "Phone Type borrado con exito." if @phone_type.destroy
    respond_with(@phone_type)
  end
end
