...
unless ARGV.size == 3
  ...
  $stderr.puts '-' * 40
  $stderr.puts "USAGE: rails runner #{$0} [company_id] [cvs_for_movements] [cvs_for_accounts]"
  exit(1)
end

company_id, cvs_for_movements, cvs_for_accounts = ARGV
...
ActiveRecord::Base.transaction do
  begin
    ...
    exercise = Exercise.find_or_create_by_company_id_and_started_on_and_finished_on(
                 ...
                 :started_on => Apslabs::Migrator::Model::Movement.minimum(:exercise_date_on),
                 :finished_on => Apslabs::Migrator::Model::Movement.maximum(:exercise_date_on),
