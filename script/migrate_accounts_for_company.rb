...
CSV.foreach(file_to_migrate) do |row|
	...
	puts "retrive code: #{code}, name: #{name}, parent_code: #{parent_code} ..."
	unless parent_code.blank? || parent_code.to_i.zero? 
	  ...
	  puts "ERROR: parent error code for #{parent_code} on #{name.try(:titlecase)} account and code #{code}!" if parent_account.nil?
	end

	puts "creating account #{name.try(:titlecase)} ..."
	account = Account.create(:company_id => company_id, :code => code, :name => (name.encode.try(:titlecase) || code))
	
	unless parent_account.nil?
		puts "moving account #{account.name.titlecase} to child of #{parent_account.name.try(:titlecase)}..."
